...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        for _ in range(num_blocks):
            self.attn.append(self.TransformerBlock(model_dim, num_heads))
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        x = self.word_embedding(context) + self.position_embedding(torch.arange(context.shape[1]))
        x = self.linear(self.ln(self.attn(x)))
        return x
