...
class CachedAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, kv_cache: Optional[KVCache] = None) -> Tuple[torch.Tensor, KVCache]:
        ...
        # x is a token, (1,1,x).
        # O(T^2) -> O(T)/token for inference, saves masks (1x1).
        q = self.q_proj(x)
        k = self.k_proj(x)
        v = self.v_proj(x)

        full_k, full_v = kv_cache.update(k, v)
        return torch.softmax(q @ full_k.transpose(-1, -2) * full_k.shape[-1] ** -0.5, dim=-1) @ full_v, kv_cache
